...
class LocketAPI:
    ...
    def _post_with_retry(self, url, headers, json_data=None, data=None, max_retries=3):
        """Hàm hỗ trợ gửi request tự động thử lại, tối ưu thời gian chờ cho lỗi Ingest 529."""
        for attempt in range(max_retries):
            try:
                response = requests.post(
                    ...
                    json=json_data,
                    data=data,
                    timeout=20  # Timeout kết nối 20s
                )
                if response.ok:
                    return response.json()
                else:
                    print(f"[!] Lỗi API (Lần {attempt + 1}/{max_retries}): {response.status_code} - {response.text}")
                    
                    # Nếu gặp lỗi 529 (đang nạp token), chờ 30 giây để Apple xử lý xong
                    if response.status_code == 529 or "ingested" in response.text:
                        print(f"[⏳] RevenueCat đang đồng bộ Token với Apple, chờ 30 giây...")
                        time.sleep(30)
                    else:
                        time.sleep(3)  # Lỗi khác thì chờ 3 giây
                        
            except requests.exceptions.RequestException as e:
                print(f"[!] Lỗi kết nối/Timeout (Lần {attempt + 1}): {str(e)}")
                time.sleep(3)

        raise Exception("API request thất bại sau nhiều lần thử.")

    def getUserByUsername(self, username):
        if not username:
            raise ValueError("Username is required")

        request_payload = {"data": {"username": username}}
    ...
    def restorePurchase(self, uid):
        ...
        # Cập nhật thời gian thực để tránh bị lỗi reject do lệch timestamp
        ...
        return self._post_with_retry(url, headers=headers, data=payload)

    def changeNameAccount(self, last="", first=""):
        """Changes the first and last name of the account."""
        request_payload = {"data": {"last_name": last, "first_name": first}}
        return self._post_with_retry(
    ...
    def GetAccountInfo(self):
        ...
        return self._post_with_retry(url, headers=headers, json_data=request_payload)
